...
Subsequently, a thorough analysis of the three validation cases is detailed.
These cases contributed significantly to the selection of the best material parameter set and to
evaluating the applicability of the material parameters in other use cases.
...
juxtaposed with the experimental data, and the RMSE and NRMSE (Equation \ref{eq:rmse} and \ref{eq:nrmse}) were calculated to quantify the quality 
...
RRMSE were useful when considering relative errors. These metrics weighed errors in relation
to the actual size of the actual values \cite{Rajagukguk2020}.\\

The simulation results that produced the lowest performance metrics were selected as possible
...
analyzing the influence of this material parameter on validation cases.
The parameters that were determined to provide the best fit to the 
experimental data based on these assessments were listed in Table \ref{tab:materialsetbestfit}.
\begin{table}[ht!]
    \centering
...
Defining satisfactory performance metrics values required a multidimensional approach. 
...
\section{Validation of the Computational Model}
\label{section:validationcm}
To confirm the reliability and effectiveness of the identified material parameters,
...
was to understand the accuracy in predicting the actual deformation shape of the specimen under 
...
The load-displacement graph indicated that the experimental data displayed again a steeper initial slope 
...
throughout the indentation until reaching the maximum depth of $\SI{4}{\milli \meter}$ (Appendix \ref{AppendixA}).\\

\begin{figure}%
	\centering
   \quad
   \includegraphics[width=10cm]{Images/validationcase/defprof/defprofdiag.png}%
   \caption[Deformation profile - Diagram]{Deformation profile measurement diagram for the second validation case, showing the measured points parallel to the x-axis.}%
...
the remeshing process of the nonlinear adaptive region meshing option utilized in the model. This
...
showed a misalignment with the experimental data, the position of maximum deformation,
Point \SI{5}{}, adjacent to the middle point, displayed a close match for sets \SI{1}{} and \SI{3}{}.

Further, as the incompressibility parameter decreased, the overall deformation decreased. The 
NRMSE values for this case were listed in Table \ref{tab:nrmsedefprof}. The NRMSE results 
revealed that the set \SI{3}{} achieved the best value, with set \SI{1}{} following closely behind. This validation case 
...
was identified as the best overall choice. This material parameter set captured with strong 
...
Except for these conditions, the blue point model followed the process as computational model II, described 
...
Furthermore, it was anticipated based on previous nearby point experiments that force components in the 
x-direction and y-direction would become significant the further the indentation position was from the middle point (Subsection \ref{subsection:nearbypointresult}).
...
On the contrary, the y-force component showed a clear nonlinear trend in the computational model,
whereas the experimental curve showed minimal variation. The z-force component was previously analyzed 
and used to identify the material parameters; therefore, this exhibited the closest match between the curves.

The total force curve exhibited a similar initial slope difference as showcased for the z-force component.
However, after reaching a displacement of $u\approx\SI{1.2}{\milli \m}$, the simulation curve showed an increased slope,
...
In the table, it was observed that the components for x and y showed high NRMSE values, but 
...
\end{figure}
%blue point
The blue point case a showcased a greater alignment across the x and y-compo-\\nents, and the total force reaction, but
...
maximum forces obtained experimentally, except for the y-component, as 
illustrated in Figure \ref{fig:bpforcecompvc}.
This indicated that the identified parameters in set \SI{3}{} were suitable 
for the blue point case as well, in addition to displaying more stable results for 
...
could model the behavior of the polyurethane specimen under indentation, particularly
if considering different locations on the specimen's surface and analyzing all 
force components.
%--------------------------------------------------------------------------------------------------
...
On the experimental design side, the physical indentation experiments were designed and performed (experimental 
...
An important inquiry at this stage was whether the selected material law adequately captured the mechanical 
...
Once the simulation captured the mechanical behavior, the material parameters' or design parameters were input into a 
response surface optimization process. Here, the definition of an adequate range, played an essential role 
...
and calculating their pre-established error metric. This error metric, such as NRMSE, was used to determine 
whether these simulations met or surpassed the established error threshold. If the simulation did not meet the threshold, 
...
difficult with a larger indentation due to the high distortion of the mesh elements. This
...
Assumptions drawn in this study also contributed to certain limitations. The assumed symmetrical
...
simplicity and avoiding external factors that could influence in the selection of the material 
model. Nonetheless, the middle point case was not optimal to accurately capturing all force reaction 
components.\\
...
suggesting the potential for further optimization of the range input in the RSO process. 
...
of balancing simplicity and accuracy in the material parameter identification process for ultra-soft polyurethane. 
...
These findings can provide useful insights and a valuable foundation for additional refinement and 
