...
indentation is performed in the direction normal to the contact surface to prevent slippage (Fig. \ref{fig:loadingdiag}).
 ...
 \begin{figure}%
    \centering
   \subfloat[\centering Indentation test configuration front view \label{fig:frontexp2}]{{\includegraphics[width=6.5cm]{Images/Experiment/secondexp1.jpg}}}%
   \qquad
...
The specimen same as in the first experimental model (Section \ref{section:expmod1}) possessed the 
same dimensions, a minor radius $r_1 = \SI{35}{\milli \m}$ and a major radius $r_2 = \SI{60}{\milli \m}$, and was
...
The maximum load capacity of the load cell is 
and the indenter is a sphere with radius of $r_s = \SI{3}{\milli \m}$. The indenter 
was made from ruby with high rigidity.
...
The indentation were conducted for each loading points 5 times on the same sample 
and the average of the results were calculated.

%write about the advantage of recording
