...
The indenter had a rounded head fabricated of stainless steel with a
...
In this experimental model (EM II), the indentation loading system gathered data of the indentation 
...
The loading system is operated by specifying the movement of the loading rod in advance, and the 
indentation is performed in the direction normal to the contact surface to prevent slippage (Fig. \ref{fig:loadingdiag}).\\
 ...
 \begin{figure}%
   ...
   \caption[Experimental model II]{YNU experimental model: 6-axis sensor test specimen constructed from ultra-soft polyurethane resin positioned on a fixed platform with a similar shape for constraint.}%
...
produced from ultra-soft polyurethane resin. Consequently, the platform is also 
...
Young's Modulus of $E_{i2} = \SI{440}{\giga \pascal}$ and a Poisson's ratio of 
...
The second experimental model offered some advantages over the first model. 
...
as this experimental setup showed some inaccuracies in the first data sample.
...
to be an appropriate compromise that would allow to capture the nonlinear 
...
Building on the previous test point (Fig. \ref{fig:midpoint}), an indentation
 test was conducted on the same point, with the an indentation depth of 
...
setup. 
%Due to time constraints, the modification of experimental model I (EM I)
...
The result of this experiment was used to guide this decision, which had important 
...
\begin{enumerate}
    ...
    \item Nonlinear plastic (failing): With the increase of the external load, the material reaches its yield point and undergoes permanent deformation
...
force reactions in the x-direction and y-direction could be disregard, as these were considerably lower than
...
The hysteresis displayed for both configurations could have occurred due to several reasons, such 
...
tested points. In x-direction, the maximum force was observe for point \SI{2}{}, which had the smallest gradient 
...
For the middle point and point \SI{1}{}, which had the largest contact surface gradient, the 
x-component force reaction was close to $\SI{0}{\newton}$. 
...
similar results point \SI{1}{} and point \SI{3}{}. For all the points the results in z-direction are more significant than 
...
In conclusion it is possible to confirm that the material behavior was 
...
\begin{enumerate}
    ...
    \item The viscoelasticity of the material can be ignored for the first design of the material model, as the material returns to its original shape and essentially no energy is lost during deformation. 
    \item Friction and shear stresses will be neglected, as in the middle point case the observed force components in x-direction and y-direction are non-relevant.
\end{enumerate}
...
\begin{align}
    C_1 = \frac{\mu}{2} \, ,
\end{align}
where $\mu$ is the shear modulus or the second Lamé parameter was chosen. If the material is assumed to be incompressible,
\begin{align}
    J = 1 \, ,
\end{align} 
the second term in the strain energy potential $W$ becomes zero \cite{Pence2015}. 
