...
of material properties, as these describe a relationship that respresents how a material behaves, e.g, 
...
Hyperelastic models are useful due to their ease of use and calibration, accesibility in major commercial FE softwares, 
...
and large strain predictions. These models have been widely develop over the years and 
...
solid, rubber-like materualas characterized by an almost incompressible behavior. 
Because of this characteristic, the actual value has very small effect on the response 
...
This equation is not adequate for the accurate description of large-strain nonlinear responses. 
In addition, this equation is not dependant from the second invariant $I_2^*$, which limits the 
...
(Eq. \ref{eq:stressNHalt}) becomes different with the decrease of the bulk modulus.
The main advantage NH model lies in its simplicity; with shear modulus known, the response of most any loading 
...
The Neo-Hookean model have been employed in various research papers to characterize soft tissue behavior. 
For instance, in a study from Kanyanta and Ivankovic (2010) used and compare different hyperelastic models to characterize different loadings
...
assumin an isotropic, incompressible model. This knowledge contributes to the identification of local biomechanical properties 
...
Likewise, this model proved to be appropiate to simulate the nonlinear properties of breast soft tissues deformation 
during a leaning forward position and running movement. By identifying the breast properties the bra-breast contact mechanism 
could be analyzed \cite{Sun2019}. 

\textbf{Yeoh Model}\\

The Yeoh model (YM) is another hyperelastic model that utilizes a Helmholtz free energy 
in a third-order polynomial in $I_1^*$ and independent of $I_2^*$. This allows the the model to provide 
...
\end{align}
The indenpendence from the $I_2^*$ term is the main motivation for the Yeoh model. Due to the difficulty of experimentally 
determining the dependence of this term with the Helmholtz free energy, the neglection of this term in the YM makes is easier that 
that the hyperelastic model is Drucker stable and therefore, facilitates its use \cite{Bergström2015}.
...
of polyurethane under different loadings. The results the Yeoh model showed the best fit to the variety of experimental data, however, 
...
The Ogden model (OM) is a highly general hyperelastic model that is determined in term of the applied principal stretches \cite{Bergström2015}.
Similar to the previous models, the Helmholtz free energy per volume can written in various ways, 
\begin{align}
        \begin{split}
...
the volumentric response is not defined by the bulk modulus, but instead $D_k$ parameters. In this equation
...
\begin{align}
        \kappa = \frac{2}{D_1} \,.
\end{align}
Equation \ref{eq:helmholtzOM} make the Ogden model versatile but can be complicated when selecting an appropiate 
...
Moreover, the OM shows often a better prediction than the Neo-Hookean and Mooney Rivlin models but is not accurate as the 
...
human skin, particularly for the plantar heel pad tissue. This reasearch led to a better understanding 
the transfer of the load during walking when the foot impacts the floor,
and it implication in the design of shoe soles \cite{Ahanchian2017}.

%Relevant papers tends to use hyperelastic models to represent soft materials as the viscoelasticity is usually neglected. 
