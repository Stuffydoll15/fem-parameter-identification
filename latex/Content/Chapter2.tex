...
\begin{figure}
        ...
        \caption{Uniaxial testing: Diagram of three tensile testing of an ether-based polyurethane elastomer specimen done with three different experiment configurations for different strain rate analysis. Diagrams are based on the experiments of made by Kayanta and Ivankovic (2010) \cite{Kanyanta2010}.}
...
\begin{figure}%
       ...
       \caption{Uniaxial compression testing: Diagram of typical compression setup and influence of interface friction on the deformed specimen shape. Illustration is based on the "Mechanics of Solid Polymers" by Bergström \cite{Bergström2015}.}%
...
\begin{figure}%
       ...
       \caption{Uniaxial compression testing: Illustration of different compression methodologies for a cartilage specimen. Unconfined compression, confined compression and Indentation. Diagrams are based on the methodologies showed by Griffin (2016) \cite{Griffin2016}.}%
...
\begin{figure}[th]
        ...
        \caption[Nanoindentation]{Nanoindentation experiment setup diagram with an indenter tip attached to a rod and a polymer as specimen. Diagram is based on the "Mechanics of Solid Polymers" by Bergström \cite{Bergström2015}.}
...
For an application with biological tissues Carter et al. performed indentation experiments on 
...
\begin{figure}[th]
        ...
        \caption[aspiration]{Aspiration experiment setup: Diagram showing aspiration tissue vertically positioned to the target tissue, inside of the tube a mirror reflects the side view of the tissue to the video camera on top. Diagram based on the research of Kauer (2002) \cite{Kauer2002}.}
...
based on the overview made by Bergström (2015), followed by some examples from the research 
...
and incompresible deformations, with the compressible version often being more 
...
Because of this characteristic, the actual value has a very small effect on the response 
...
The main advantage NH model lies in its simplicity; with the shear modulus known, the response of most any loading 
...
fiber network of the cervical stroma, to identify the $\mu$ and $\lambda$ and then calculate the Young's modulus and 
...
These parameters are defined based on the experimental data and in case of small strain the shear modulus can be represented as \cite{Youssef2022}
\begin{align}
        \mu = C_{10} + C_{01} \, .
...
Some of the research papers, which used the Mooney-Rivlin model are, e.g., Drass et al. (2018) utilized the MR 
...
Likewise, this model proved to be appropriate to simulate the nonlinear properties of breast soft tissues' deformation 
...
determining the dependence of this term with the Helmholtz free energy, the neglection of this term in the YM makes 
it easier to apply. Moreover, the hyperelastic model is Drucker stable, which another factor that facilitates the use of this model \cite{Bergström2015}.
...
of polyurethane under different loading. The results the Yeoh model showed the best fit to the variety of experimental data, however, 
...
\end{align}
being this equation, one of the most common for a compressible Ogden Model. Different from the previous presented models, is that 
...
\end{align}
and the stresses for incompressible OM in unixial, planar and biaxial loading are given by
\begin{align}
...
due to their direct application and capacity to describe nonlinear behavior for small and large strain.
While these models have their respective advantages and limitations, they provide a basis knowledge of 
complex material behavior and serve as foundation for more advance models. The selection of the models 
...
with an optimized set of material parameters, and extract and analyze more informatio about the material's behavior \cite{Kauer2002}.
...
\begin{enumerate}
        ...
        \item \textbf{Finite element model development}: A finite element model is developed, which includes the definition of the geometry design, contact definition, boundary conditions and the selcetion of the appropriate material model \cite{Jamal2019}.
        ...
        \item \textbf{Optimization of the parameters}: With the aid of an optimization algorithm, the material parameters are iteratively calculated until the simulated response matches or reaches a good agreement with the experimental response. The Levenberg-Marquardt and the genetic algorithm are common algorithm employed in this step \cite{Kauer2002}.
...
Some of the strengths of the iFEM is that this method allows the identification of parameters which are difficult to obtain 
...
assumption of an homogeneous material, which helps in the description of local stresses ans strain fields \cite{Seshaiyer2003}.\\

On the other hand, iFEM presents several limitations and challenges, such as the requirement for accurate and reliable 
data,  the possibility of non-unique solutions when the optimization is ill-defined, and the  inability to find a 
unique set of parameters to describe all regions of heterogeneous materials. Additionally, iFEM often needs
high computational costs and requires a robust optimization algorithm \cite{Giudice2021}.\\
...
The FE model of the aspiration method (deformation of the tissue on the aspirated area) required the contact 
...
\end{align*}
was selected for the explicit displacement-pressure finite element formulations %\cite{SUSSMAN1987}. This function depends on the reduced invariants $J_1$, $J_2$, $J_3$ of the right Cauchy-Green 
...
of three parameters: the shear modulus, and the bulk modulus. This method did not guarantee obtaining a global minimum, however after the selection of arbitriarily 
...
Ahanchian (2017) characterized the the behavior of heel pad sub-layers utilizing the inverse finite element method.
%Experiment
With the aid of the ultrasound imaging and the indentation of the plantar heel pad tissue force-strain responses were recolected 
