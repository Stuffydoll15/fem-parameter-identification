...
these methods possess limitations in cases where quantifiable data is necessary, specifically for computer-assisted systems, e.g,
medical diagnosis, thera-\\py, and training \cite{Kauer2002}.
To gather the material data, e.g., elasticity, stiffness, response under deformation and temperature, it is required 
to gain access to the soft tissues and perform \textit{in vivo} or \textit{ex vivo} testing experiments. 
...
%is that material properties may change despite examination of the same sample. Their biomechanical properties 
...
will be explained. The goal of this chapter is to provide a comprehensive overview of the current methods use in 
...
To characterize the mechanical behavior of a test specimen, the most common
...
%modulus range (kPa) present challenges for the design of experimental testing \cite{Liu2009}.

\subsubsection*{Uniaxial Tension Testing}

Uniaxial tension testing is widely employed to determine a stress-strain relationship.
For uniaxial tension cases, a specimen is loaded by gripping the ends while applying 
tension, and the deformation is measured with a strain gauge \cite{Bergström2015}.
This kind of testing focuses on the central region of the specimen to evade complications arising from 
edge effects. A homogeneous deformation in the central region is expected for this kind of testing, 
which simplifies the boundary problem and ensures that the measurements represent valid stress-strain values.
However, there are two key limitations for uniaxial testing when testing soft materials; first, 
it may not be suitable when complex boundary conditions arise, and it is not possible to control the 
...
\end{align}
were calculated. Here, $A_b$ is the bar's cross-sectional area, $A_s$ the specimen's cross-sectional area,
...
modulus values for the dry-room temperature setup possessed a Young's modulus of $\SI{7.4}{\mega \pascal}$, decreasing to $\SI{5.3}{\mega \pascal}$ 
...
Young's modulus was insignificant and, hence, this could be assumed to be constant. This study demonstrated 
that it is important to measure the properties of the elastomer under similar conditions to the
intended application, as properties vary under different conditions.
...
involve placing the specimen in between two plates and compressing the material (Fig. \ref{fig:compressiondiag}).
...
was used for this test with a saBesto HHS 5000 machine. The compression test was conducted until a strain $\epsilon=\SI{0.6}{}$ 
...
pressed out between the test specimen and the pressure plates, resulting in an increased friction. 

The results of this experiment were processed to identify hyperelastic material parameters using standard 
fitting routines and inverse methods. The test suggested that only stress-strain responses up to a strain value of 
...
In comparison, for biomaterials, e.g., human soft tissues, a compressive testing of cartilage was conducted by \citet{Griffin2016}. 
This study aimed to provide a protocol where compressive and tensile properties of human soft tissues can be evaluated 
...
The mechanical response of cartilage is highly dependent on the fluid's flow through the tissue. The methods for compression 
testing can vary with confined or unconfined specimen, and the most prevalent, indentation (Fig. \ref{fig:compressiontypes}).
In the unconfined compression, the cartilage was pressured using a non-porous plate onto a non-porous chamber, 
...
uniaxial compression and minimizing shear loading. All tests were conducted in a 
hydrated environment, and the cartilage was submerged in phosphate-buffered saline before and during the test to maintain 
the hydration. With the latest compression testing type, it was possible to identify elastic and viscoelastic properties 
...
uniaxial or biaxial testing, cannot to be employed, 
...
The usual indentation testing setup is shown in Figure \ref{fig:Nanoindentation}, where a systems applies a 
certain force through an indenter rod with an specific indenter tip.
After the indenter tip goes to a determined displacement, 
it is possible to obtain a load-displacement curve.
The deformation can be measured through a capacitance gauge or also optically, via laser measurements \cite{Bergström2015}.\\
...
that was prepared and cured in a cylindrical mold and cured at $\SI{60}{\degreeCelsius}$ for eight hours. 
The ElectroForce 3100 was used to measure the mechanical properties of PDMS for tensile and 
...
For the indentation tests, a spherical indenter with a $\SI{3}{\milli \m}$ radius with a similar configuration 
...
an indentation depth of $\SI{3}{\milli \m}$. Six measurements were carried out at different 
...
certain depth dependence could be observed. However, a locking stretch could not be analyzed due to the 
...
manually with a winding mechanism at a constant rate of $\SI[per-mode = symbol]{1}{\milli \m\per \second}$ 
...
the average of the repeated measured was calculated. The measured elastic moduli of the pig 
spleen and liver were $\SI{0.11}{\mega \pascal}$ and $\SI{4}{\mega \pascal}$ respectively, and 
for the human liver about $\SI{0.27}{\mega \pascal}$. However, a 
...
The biological soft tissue used in this paper was a human uterus. This tissue was selected 
because it possessed a complex, multilayered structure with anisotropic properties. 
Moreover, hysterectomy (removal of the uterus) is a common surgical procedure, which 
provided a solid opportunity to perform measurements before and after the removal of the organ.\\

This method introduced an aspiration tube which was put against the 
soft tissue, generating a vacuum and causing the deformation of the tissue (Fig. \ref{fig:aspiration}). 
An advantageous feature of this experimental technique is that 
it can be performed \textit{in vivo} and \textit{ex vivo}.
With the help of a mirror placed next to the aspiration hole, the reflection 
of the side-view of the tissue could be captured with a video camera.
This camera captured the images of the illuminated surface of the material, and the 
aspiration pressure was captured through a sensor. Through this process, the captured 
profile of the tissue was obtained and this was used to characterize the deformation 
...
properties of the tissue for real surgical procedures. This is due to 
the time-dependent resolution of the deformation.\\

The results from these experiments corroborate that the development of experimental designs for
...
of material properties. These describe a relationship that represents how a material behaves, e.g., 
the stress response for an applied strain, or the heat transfer for a defined temperature gradient.
For soft materials, hyperelastic models are  
...
accurate for monotonic loading, these models possess limitations, such as not capturing rate effects, i.e., viscoelasticity, or 
...
$\SI{1}{}$, $\SI{2}{}$, and $\SI{3}{}$, and $\epsilon_{ij}$ and $\sigma_{ij}$ are the strain and stress tensors, where
\begin{align}
...
and the Lamé's constant $\lambda$. 
...
behavior of the material, as is generally the case for biopolymers. The strain $\epsilon_{ij}$ and stress $\sigma_{ij}$ 
\begin{align}
...
are available in various FE softwares. Hyperelastic models are essential in modeling 
...
solid, rubber-like materials characterized by a nearly incompressible behavior. 
Because of this characteristic, the actual value shows minimal effect on the response 
...
The Cauchy stress for compressible NH model can be expressed as 
\begin{align}
        \sigma = \frac{\mu}{J}\text{dev}[\boldsymbol{b}^*] + \kappa(J-1)\boldsymbol{I} \, ,
        \label{eq:cauchystressNHcomp}
\end{align}
where $\boldsymbol{b}^*$ is the left Cauchy-Green strain tensor, and $\boldsymbol{I}$ is the identity matrix \cite{Youssef2022}. 
...
be an isotropic and incompressible material for the description of the behavior of polyurethane \cite{Kanyanta2010}.\\

Furthermore, Chai et al. (2013) utilized the Neo-Hookean model to calculate the Young's modulus of human carotid plaques
assuming an isotropic, incompressible model. This knowledge contributed to the identification of local biomechanical properties 
...
fiber network of the cervical stroma. By using this model, it was possible to identify the $\mu$ and $\lambda$ and then calculate the Young's modulus and 
...
\end{align}
The independence from the $I_2^*$ term is the main motivation to use the Yeoh model. Due to the difficulty of experimentally 
determining the dependence of the $I_2^*$ term with the Helmholtz free energy, disregarding this term in the YM simplifies its application.
...
the Neo-Hookean model showed to be adequate for small strain deformations \cite{Kanyanta2010}.
Additionally, Łagan et al. (2017) used the Yeoh model to analyze the accuracy of model fitting 
to the experimental data and estimated the mechanical properties of swine skin tissue in the abdominal region under uniaxial testing \cite{Lagan2017}.\\
...
While these models exhibited their own set of advantages and limitations, they provided a basic level of understanding of 
complex material behavior and served as a foundation for more advanced models. The selection of the models 
...
Furthermore, this approach aims to predict additional experimental results with a validated computational model 
...
\begin{enumerate}
        \item \textbf{Experimental data collection}: Experimental data of the observed material is gathered from the conducted mechanical tests, e.g., uniaxial tensile and compression testing, indentation, etc. This data, often given as a load-displacement curve, is the basis for the identification of the parameters \cite{Seshaiyer2003}.
        ...
        \item \textbf{Optimization of the parameters}: With an optimization algorithm, the material parameters are iteratively calculated until the simulated response matches or reaches a close agreement with the experimental response. The Levenberg-Marquardt and the genetic algorithm are common algorithms employed in this step \cite{Kauer2002}.
...
through conventional methods and conserves the native geometry of the specimen. Moreover, it is particularly advantageous for the 
...
data, the potential for non-unique solutions in case of poorly defined optimization, and the inability to find a 
...
inverse finite element approach before applying it to more challenging biomaterials, where \textit{in vivo} or \textit{ex vivo} measurements are required.\\
...
and the FE model of the deformation of the tissue on the aspirated area, required the contact definition
between the tube and the tissue set as sticking. The strain energy function 
\begin{align}
...
This function depended on the reduced invariants $J_1$, $J_2$, $J_3$ of the right Cauchy-Green 
deformation tensor $\boldsymbol{C}$, where $\mu_i$ and $\kappa$ were the target material parameters. For $N = \SI{1}{}$ a nearly incompressible Neo-Hookean model 
was formulated, which was chosen for the case of silgel. The Levenberg-Marquardt optimization method was selected for the optimization 
...
actual tensile tests. The prediction of the synthetic tissue demonstrated accurate prediction quality with these parameters \cite{Kauer2002}.
...
\end{align}
was calculated, where $F_s$ and $F_e$ were the simulated and experimental forces, respectively. The RMSE was the objective function to be minimized, 
...
the element high distortion issue.\\
...
and the results obtained from the iFEM. Additionally the validation of the best material parameters are presented. 
This chapter assesses the accuracy of the identified parameters by comparing them with other validation experiments.\\
